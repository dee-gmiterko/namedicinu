...
export default class FieldDetail extends Component {
  ...
  render() {
    ...
    field.descriptionPanel.forEach((item) => {
      ...
      if (item.key) {
        descriptionPanelCategories[item.category].items.push({
          key: item.key,
          value: item.value ? fixNbsp(item.value) : null,
        });
      } else {
    ...
    const descriptionPanelItems = Object.keys(descriptionPanelCategories).map(
      (categoryTitle, index) => {
        ...
        <Table responsive bordered size="sm" className="text-center students">
          <thead>
            <tr>
              <th>
                {!field.faculty.dentistry && (
                  <FormattedMessage
                    id="faculties_comparison.students.year"
                    defaultMessage="Year"
                  />
                )}
              </th>
              <th colSpan={2}>
                <FormattedMessage
                  id="faculties_comparison.students.signed"
                  defaultMessage="Signerd"
                />
              </th>
              <th colSpan={2}>
                <FormattedMessage
                  id="faculties_comparison.students.accepted"
            ...
            {field.faculty.dentistry && (
              <tr>
                ...
                <th>
                  <FormattedMessage
                    id="faculties_comparison.students.general"
                    defaultMessage="General"
                  />
                </th>
                <th>
                  <FormattedMessage
                    id="faculties_comparison.students.dentistry"
                    defaultMessage="Dentistry"
                  />
                </th>
                {field.faculty.dentistry && (
                  <>
          ...
          <tbody>
            {field.faculties_students
              ...
              .map((students, index) => {
                const fourColumns =
                  students.dentistrySigned || students.dentistryAccepted;
                return (
                  <tr key={index}>
                    <th>{students.year}</th>
                    ...
                    <td colSpan={fourColumns ? 1 : 2}>
                      {students.generalAccepted && (
                        <span>
                          <FormattedNumber value={students.generalAccepted} />
                        </span>
                      )}
                      {students.generalAccepted && students.generalSigned && (
                        <span className="percent-accepted">
                          {" "}
                          (
                          {Math.round(
                            (students.generalAccepted * 100) /
                              students.generalSigned
                          )}{" "}
                          %)
                        </span>
                    ...
                    {fourColumns && (
                      <td>
                        {students.dentistryAccepted && (
                          <span>
                            <FormattedNumber
                              value={students.dentistryAccepted}
                            />
                          </span>
                        )}
                        {students.dentistryAccepted &&
                          students.dentistrySigned && (
                            <span className="percent-accepted">
                              ...
                              (
                              {Math.round(
                                (students.dentistryAccepted * 100) /
                                  students.dentistrySigned
                              )}{" "}
                              %)
                            </span>
        ...
        <Row className="mb-4">
          <Col md={7}>
            ...
            <p className="updated-at">
              <FormattedMessage
                id="faculties_comparison.updated_at"
                defaultMessage="Lecturers"
              />
              &nbsp;
              <FormattedDate
          ...
          <Col md={5}>
            ...
            <div>
              <Button className="btn-find-course" as={AnchorLink} to="/#Course">
                <FormattedMessage
                  id="faculties_comparison.courses"
