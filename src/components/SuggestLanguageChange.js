...
const SuggestLanguageChange = ({ locale }) => {
  ...
  useEffect(() => {
    ...
    let preferred = preferredLocale(expected, ["sk-sk", "cs-cz"], {
  ...
  const otherWebsite =
    locale === "sk" ? `https://namedicinu.cz/` : `https://namedicinu.sk/`;
    ...
    <Modal show={show} onHide={handleClose} dialogClassName="language-modal">
      ...
      <Modal.Body>
        <Row className="pl-5 pr-5">
          <FormattedMessage
            id="language_change.description"
            defaultMessage=""
          />
        </Row>
      </Modal.Body>
