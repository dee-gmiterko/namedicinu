...
const FormPaymentMethod = () => {
  ...
  const deposit =
    courses > 0 && product.price.length >= courses
      ? product.price[priceIndex].deposit
      : 0;
  const formattedDeposit = intl.formatNumber(deposit, priceStyle);
  const variationDate =
    variation &&
    variation.length > 0 &&
    moment(variation.split(" ")[0], ["DD.MM.YYYY"]);
  const depositDueDate = product.registerEnd
    ? moment(
        Math.min(
          moment().add(14, "days").valueOf(),
          moment("2023-09-15").valueOf()
        )
      )
    : variationDate
    ? variationDate.subtract(2, "days")
    : moment();
                      ...
                      <Form.Check.Label>
                        ...
                        <Markdown
                          ...
                          params={{
                            ...
                            depositDueDate: intl.formatDate(depositDueDate, {
                              year: "numeric",
                              month: "long",
                              day: "numeric",
                            }),
                            payment: intl.formatNumber(
                              (price - deposit) / pf.portions,
                              priceStyle
                            ),
                          }}
                        />
