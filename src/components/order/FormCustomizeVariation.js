...
import { fixNbsp, slugifyDocumentTitle } from "../../utils";
...
function FormCustomizeVariation() {
  ...
  const filteredVariations = product.product_variation
    ? (
        product.product_variation.find(
          (item) => slugifyDocumentTitle(item.faculty.title) === faculty
        ) || { variations: [] }
      ).variations
    : null;
  ...
  return (
    <>
      {!product.product_variation && (
        <Row>
          <Form.Group as={Col} md={6} controlId="registerPreferredTime">
            <FormattedMessage
              id="register.preferred_time"
              defaultMessage="Preferred time"
            >
              {(l_preferred_time) => (
                <>
                  <Form.Label>{l_preferred_time}</Form.Label>
                  <Form.Control
                    name="preferred_time"
                    as="select"
                  ...
                  >
                    <option value=""></option>
                    <FormattedMessage
                      id="register.preferred_time.morning"
                      defaultMessage="Morning"
                    >
                      {(o) => <option value={o}>{o}</option>}
                    </FormattedMessage>
                    <FormattedMessage
                      id="register.preferred_time.afternoon"
          ...
          <Form.Group as={Col} md={6} controlId="registerPreferredeDay">
            <FormattedMessage
              id="register.preferred_day"
              defaultMessage="Preferred day"
            >
              {(l_preferred_day) => (
                <>
                  <Form.Label>{l_preferred_day}</Form.Label>
                  <Form.Control
                    name="preferred_day"
                    as="select"
                    disabled={formDisabled}
                  >
                    <option value=""></option>
                    <FormattedMessage
                      id="register.preferred_day.workday"
                      defaultMessage="Workday"
                    >
                      {(o) => <option value={o}>{o}</option>}
                    </FormattedMessage>
                    <FormattedMessage
                      id="register.preferred_day.weekend"
      ...
      <Form.Group controlId="registerFaculty">
        <FormattedMessage
          id="register.faculty.with_variation"
          defaultMessage="Faculty"
        >
          {(l_faculty) => (
            <>
              <Form.Label>{l_faculty}</Form.Label>
              <Form.Control
                name="faculty"
                as="select"
                onChange={(event) => {
              ...
              >
                ...
                {displayFaculties.map((faculty, index) => {
                  return (
                    <option
                      key={index}
                      value={slugifyDocumentTitle(faculty.title)}
                    >
      ...
      <Form.Group controlId="registerVariation">
        <FormattedMessage id="register.variation" defaultMessage="Variation">
          {(l_variation) => (
            <>
              <Form.Label>{l_variation}</Form.Label>
              <Form.Control
                name="variation"
                as="select"
                disabled={formDisabled}
